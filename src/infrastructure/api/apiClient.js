...
const getMockUsers = () =>
  JSON.parse(localStorage.getItem("sentinel_mock_users") || "[]");
const saveMockUsers = (users) =>
  localStorage.setItem("sentinel_mock_users", JSON.stringify(users));

const getMockHistory = (userId) =>
  JSON.parse(localStorage.getItem(`sentinel_mock_history_${userId}`) || "[]");
const saveMockHistory = (userId, history) =>
  localStorage.setItem(
    `sentinel_mock_history_${userId}`,
...
const mockAdapter = async (config) => {
  ...
  if (url.includes("/api/detection/detect")) {
    ...
    const contentType = data.contentType || 1;

    // Vary AI probability based on content type
    // Audio (4) and Video (3) tend to show higher AI probability
    // Text (1) and Image (2) are more mixed
    let aiProbability;
    if (contentType === 4) {
      // Audio: random between 25-75 (varied, not always human)
      aiProbability = Math.floor(Math.random() * 50) + 25;
    } else if (contentType === 3) {
      // Video: random between 30-80
      aiProbability = Math.floor(Math.random() * 50) + 30;
    } else {
      // Text and Image: original logic
      const isAi =
    ...
    const userId =
      authHeader
        .replace("Bearer mock-jwt-token-", "")
        .replace("Bearer mock-guest-token-", "") || "user-default-1";
      ...
      classification:
        aiProbability > 60
          ...
          : aiProbability > 30
            ? "Mixed content"
            : "Proven Human",
      // Vary confidence based on content type: audio/video more variable, text more confident
      confidenceIndex:
        contentType === 4 || contentType === 3
          ? Math.floor(Math.random() * 35) + 60 // Audio/Video: 60-95
          : Math.floor(Math.random() * 15) + 80, // Text/Image: 80-95
